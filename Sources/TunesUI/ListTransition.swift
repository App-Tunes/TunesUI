...
public extension ListTransition {
	...
	func executeAnimationsInOutlineView(_ outlineView: NSOutlineView, childrenOf parent: Any?) {
		...
		case .reload(let previousSize, let newSize):
			...
			let reloadItems = IndexSet(integersIn: 0 ..< min(previousSize, newSize))
			outlineView.removeItems(at: reloadItems, inParent: parent, withAnimation: [])
			outlineView.insertItems(at: reloadItems, inParent: parent, withAnimation: [])
		}
	}
}
